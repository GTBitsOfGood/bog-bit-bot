...
app.message("checkin", async ({ message, say }) => {
  ...
  if (await bcrypt.compare(messageArray[1], passConfig.password)) {
    ...
    await botReply(
      `Checked in. You now have ${user.bits + 1} bits!`,
      message,
      say
    );
    if (formConfig) {
      await botReply(
        `Please visit ${formConfig.form} to fill out today's form.`,
        message,
        say
      );
    }
...
app.message("set password", async ({ message, say }) => {
  ...
  await config.updateOne({ password: { $exists: true } }, updateDoc, {
    upsert: true,
  });
...
app.message("set role", async ({ message, say }) => {
  ...
  if (messageArray.length < 2) {
    return await botReply(
      "Please provide a user. Remember to use @username.",
      message,
      say
    );
  }
  ...
  if (params.length < 2) {
    return await botReply(
      "Please provide a role. Available roles are exec, leader, member",
      message,
      say
    );
  }
  ...
  if (newRole != "exec" && newRole != "leader" && newRole != "member") {
    return await botReply(
      "Incorrect role. Available roles are exec, leader, member.",
      message,
      say
    );
  }
...
app.message("give team_bits", async ({ message, say }) => {
  ...
  if (messageArray.length < 3) {
    return await botReply(
      "Please provide a team. Check the team list through `set team`.",
      message,
      say
    );
...
app.message("set form", async ({ message, say }) => {
  ...
  await config.updateOne({ form: { $exists: true } }, updateDoc, {
    upsert: true,
  });
...
app.action("change_team", async ({ body, ack, say }) => {
  ...
  await ack();
  let newTeam = body.actions[0].selected_option.value;
